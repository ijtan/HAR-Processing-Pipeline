...
\caption*{Dynamic(left) and Static(right) UCI Model Confusion Matrix}
...
\includegraphics[width=.2\textwidth]{OD_Static_Confusion_Matrix}\hfill
\caption*{Dynamic(left) and Static(right) OD Model Confusion Matrix}
\label{OD_Confusions}
\end{figure}
...
The Dynamic model achieved good results with an accuracy of >60\%, when considering it trained on a reduced sized dataset with an equal distribution of 15\% for each label.
...
The Static model achieved very high results (>90\%).
